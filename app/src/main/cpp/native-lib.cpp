...
//    }
    ...
    for(;;){
        ...
        if(re!=0){
            ...
            LOGW("video end %d,%d,%d",ic->streams[vS]->time_base.den,ic->streams[vS]->time_base.num,pos);
        ...
        for (;;){
            ...
            if(cc == vc){
                ...
                if(vctx==NULL){
                    ...
                    // 一行宽度  根据像素格式确定大小
                    lines[0] = outWidth*4;
                    int h =sws_scale(vctx, (const uint8_t *const *) frame->data, frame->linesize, 0, frame->height,
                                     data, lines
                    );
                    LOGW("sws_scale %d",h);
                    if(h>0){
